...
async function collect(startblock, address) {
  try {
    ...
    const data = await Etherscan.account.txlist(
      address,
      startblock,
      'latest',
      'asc'
...
function collectMetadata(txListMata, pointer, dataMapTotal) {
  ...
  if (dataMapTotal['gasHigh'] == 0) {
    dataMapTotal['gasHigh'] = gas
  }
  if (dataMapTotal['gasLow'] == 0) {
    dataMapTotal['gasLow'] = gas
  }
  if (dataMapTotal['gasHigh'] < gas) {
    dataMapTotal['gasHigh'] = gas
  }
  if (dataMapTotal['gasLow'] > gas) {
    dataMapTotal['gasLow'] = gas
  }
  dataMapTotal['gasTotal'] += gas

  tempInput = getMeaningOfInput(
    txListMata[pointer].input.toString().substring(2, 10)
  )
  ...
  if (`${tempInput} count` in dataMapTotal) {
    ...
    dataMapTotal[`${tempInput} errorCount`] += parseInt(
      txListMata[pointer].isError
    )
    dataMapTotal[`${tempInput} gasTotal`] += gas
    if (dataMapTotal[`${tempInput} gasHigh`] < gas) {
      dataMapTotal[`${tempInput} gasHigh`] = gas
    }
    if (dataMapTotal[`${tempInput} gasLow`] > gas) {
  ...
  for (var i = txCount; i < txList.length; ++i) {
    ...
    if (txList[i].timeStamp > dataMapDay.date + 86400) {
      if (dataMapDay.count != 0) {
        if (outputBool == true) {
          console.log(await timeConverter(dataMapDay.date))
        }
